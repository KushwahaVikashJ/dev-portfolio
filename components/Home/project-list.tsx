...
const Project = ({ project, index }) => {
  return (
    <Link href={project.slug} className={"col-span-full cursor-pointer "}>
      <article
        key={project._id}
        className={
        ...
        >
          ...
          {project.name && (
            <p className="mt-2 flex items-center gap-2 truncate">
              ...
              {project.status &&
                project.status.map((chip, index) => {
                  return (
                    <span
                      key={"status-" + chip + "-" + project._id + "-" + index}
                      className={
...
export default function SectionListOfProjects({ projects }) {
  return (
    <section
      id="products"
